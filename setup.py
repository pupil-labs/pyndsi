...
if platform.system() == "Darwin":
    ...
    include_dirs += ["pl_ndsi\\h264\\windows"]
...
h264_sources = glob.glob("pl_ndsi/h264/*.cpp")
if platform.system() == "Windows":
    h264_sources += glob.glob("pl_ndsi/h264/windows/*.cpp")

extensions = [
    Extension(
        name="pl_ndsi.frame",
        sources=h264_sources + ["pl_ndsi/frame.pyx"],
    ...
    Extension(
        name="pl_ndsi.writer",
        sources=h264_sources + ["pl_ndsi/writer.pyx"],
        include_dirs=[numpy.get_include()] + include_dirs,
        library_dirs=library_dirs,
        libraries=libs,
        extra_link_args=extra_link_args + ["-std=c++11"],
    ...
    Extension(
        name="pl_ndsi.sensor",
        sources=h264_sources + ["pl_ndsi/sensor.pyx"],
        include_dirs=[numpy.get_include()] + include_dirs,
        library_dirs=library_dirs,
        libraries=libs,
        extra_link_args=extra_link_args + ["-std=c++11"],
    ...
    Extension(
        name="pl_ndsi.network",
        sources=h264_sources + ["pl_ndsi/network.pyx"],
        include_dirs=[numpy.get_include()] + include_dirs,
        library_dirs=library_dirs,
        libraries=libs,
        extra_link_args=extra_link_args + ["-std=c++11"],
...
setup(
    name="pl_ndsi",
    version=find_version("pl_ndsi", "__init__.py"),
